...
public:
  ...
  AE_NO_TSAN explicit ReaderWriterQueue(size_t size = 32)
#ifndef NDEBUG
  ...
  AE_FORCEINLINE bool enqueue_fast(T const &element) AE_NO_TSAN {
  ...
  // Allocates an additional block of memory if needed.
  // Only fails (returns false) if memory allocation fails.
  AE_FORCEINLINE bool enqueue_fast(T &&element) AE_NO_TSAN {
  ...
  #if MOODYCAMEL_HAS_EMPLACE
  template <typename... Args>
  bool inner_enqueue_fast(Args &&...args) AE_NO_TSAN
#else
  template <typename U>
  bool inner_enqueue_fast(U &&element) AE_NO_TSAN
#endif
  {
#ifndef NDEBUG
    ReentrantGuard guard(this->enqueuing);
  ...
  static Block *make_block(size_t capacity) AE_NO_TSAN {
    ...
    for (size_t i = 0; i < size; i += 4096) {
        touch[i] = 0;  // 这里的 'touch' 会让操作系统加载页面
    }
    queue_size += size;
    // auto newBlockRaw = static_cast<char *>(numa_alloc_onnode(size, 0));
